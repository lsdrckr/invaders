...
void collide_mstr_missile(cellule **mstr, lutin *m){
    ...
    bool cont = 1;
    while ((tmp != NULL) && (cont)){
        if ((tmp->lut.posx <= m->posx) &&
            ((tmp->lut.posx + taille_mstr[0]) >= (m->posx + taille_m[0])) &&
            (tmp->lut.posy <= m->posy) &&
            ((tmp->lut.posy + taille_mstr[1]) >= (m->posy + taille_m[1]))){
            m->posy = -100;
            tmp->lut.etat = 0;
            cont = 0;
            break;
...
void collide_p_b(cellule **bomb, lutin *p){
    ...
    bool cont = 1;
    while ((tmp != NULL) && (cont)){
        if ((tmp->lut.posx >= p->posx) &&
            ((tmp->lut.posx + taille_bomb[0]) <= (p->posx + taille_p[0])) &&
            ((tmp->lut.posy + taille_bomb[1]) >= p->posy) &&
            (tmp->lut.posy <= (p->posy + taille_p[1]))){
            tmp->lut.etat = 0;
            p->etat = 0;
            cont = 0;
            break;
